    ...
    scope.InfiniteScroller.prototype = {
      ...
      */
      onSwipeOut_:function(event){
        ...
        for(var i = index+1; i<this.lastAttachedItem_;i++){
          this.items_[i].top = this.items_[i].top - item.height;
          this.items_[i].node.style.transform = updateTransformProp(this.items_[i].node,'translateY(' + (this.items_[i].top) + 'px) ');
        ...
        this.fill(0,this.lastAttachedItem_);
      },
      /**
      ...
      /**
       * Setup Initial  Position for Placeholder Element
       */
      setupInitialAnimationPosition: function(tombstoneAnimations){
      ...
      attachContent: function() {
        ...
        this.setupInitialAnimationPosition(tombstoneAnimations);
        for (i = this.firstAttachedItem_; i < this.lastAttachedItem_; i++) {
          var anim = tombstoneAnimations[i];
          if (anim) {
